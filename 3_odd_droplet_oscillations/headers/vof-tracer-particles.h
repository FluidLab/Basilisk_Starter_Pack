...
int reposition (scalar f, Particles P) {
  ...
  if  (Pphase[P] >= 0) { //Do something
    ...
    foreach_particle_in(P) {
      foreach_point(x, y, z, serial) { // This x, y, z correspondd to the particle coordinates
        // Point point = locate (x, y, z); 
        coord cc = {x, y, z}; // This x, y, z corresponds to the cell centre (the cell where this particle is located)
        if (f[] > 1e-6 && f[] < 1 - 1e-6) {
