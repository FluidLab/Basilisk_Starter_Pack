...
#include "bcg.h"
#if EMBED
ODD VISCOSITY IS NOT CURRENTLY IMPLEMENTED FOR EMBED BOUNDARIES!!!!
# include "viscosity-embed.h"
#else
# include "viscosity_odd.h"
#endif
...
/* New face field for the odd viscosity */
(const) face vector mu_odd = zerof;
...
event viscous_term (i++,last)
{
  if (constant(mu.x) != 0.) {
    correction (dt);
    #ifdef ODD_EXPLICIT
      mgu = viscosity_explicit (u, mu, mu_odd, rho, dt);
    #else
      mgu = viscosity (u, mu, mu_odd, rho, dt, mgu.nrelax);
    #endif
