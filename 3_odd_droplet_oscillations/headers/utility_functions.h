...
void PrintMeshVTK_ASCII(int n, double time, bool print_only_droplet,
...
void PrintMeshVTK_Binary_Double(int n, double time, bool print_only_droplet,
                                scalar *list_scalar_data, const char **list_scalar_names)
{
  FILE *arq;
  char nomeArq[900];
...
void Output_Rekick_Data(int n, double time, int nx_ny) 
{
  int i, j;
  static const float bad_data = 1e+10;

  /// === Sometimes the interpolate function that we use further below gets stuck without updatting boundaries
  scalar distance_func[];
  boundary({f});
  CalculateDistanceFunction(distance_func);
  

  double xMin = -1.5, yMin = -1.5;
  double xMax = 1.5, yMax = 1.5;
  double dx = (xMax - xMin)/nx_ny;
  double dy = (yMax - yMin)/nx_ny;

  // Allocating memory for all the scalar field
  double *values = (double *)malloc( (nx_ny+1)*(nx_ny+1)*sizeof(double) );

  int vertex_index = 0;
  for(i=0; i<=nx_ny; i++) {
    for(j=0; j<=nx_ny; j++) {
      double x_grid = xMin + i*dx;
      double y_grid = yMin + j*dy;

      double distance_local = interpolate(distance_func, x_grid, fabs(y_grid));
      values[vertex_index] = distance_local<1000 ? distance_local : bad_data;
      vertex_index++;
    }
  }
  ...
  /// Allocating memory for the arrays coming from the remote processes into the root
  #if _MPI
    double *remote_values[npe()];
    if( pid()==0 ) {
      for(int id_proc=1; id_proc<npe(); id_proc++)
        remote_values[id_proc] = (double *)malloc((nx_ny+1)*(nx_ny+1)*sizeof(double));
    }
  
    // printf("a %d kkk %d\n", scalar_index, pid());
    if( pid() )
      MPI_Send(values, (nx_ny+1)*(nx_ny+1), MPI_DOUBLE, 0, 1000 + pid(), MPI_COMM_WORLD);
    else {
      for(int id_proc=1; id_proc<npe(); id_proc++)
        MPI_Recv(remote_values[id_proc], (nx_ny+1)*(nx_ny+1), MPI_FLOAT, id_proc, 1000 + id_proc, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        
      vertex_index = 0;
      for(i=0; i<=nx_ny; i++) {
        for(j=0; j<=nx_ny; j++) {
          for(int id_proc=1; id_proc<npe() && values[vertex_index]==bad_data; id_proc++ )
            values[vertex_index] = remote_values[id_proc][vertex_index];
          vertex_index++;
        }
      }
    }
  ...
  if( pid()==0 ) {
    char fileName[900];
    sprintf(fileName, "%s/MeshVertexDump-%d.bin", folder_name, n);
    FILE *file_out = (FILE *)fopen(fileName, "w");

    // Printing a header with name of scalars and the dimensiona of the mesh (nx, ny)
    fprintf(file_out, "%d %d\n", nx_ny, nx_ny);
    fwrite(values, sizeof(double), (nx_ny+1)*(nx_ny+1), file_out);
    fclose(file_out);
  }
...
void LoadUniformDataDump_Vertices(const char *file_name, vertex scalar phi)
{
  int nx, ny;
  
  FILE *file_in = (FILE *)fopen(file_name, "r");
  if( !file_in ) {
    printf("Problem opening the data dump file.\n\n");
    exit(1);
  }
  ...
  printf("NX NY: %d %d\n", nx, ny);
  printf("dx dy %lf %lf\n", dx, dy);
  printf("xMin etc %lf %lf %lf %lf\n", xMin , xMax, yMin, yMax);




  double *mesh_f = (double *)malloc( (nx+1)*(ny+1)*sizeof(double) );
  fread(mesh_f, sizeof(double), (nx+1)*(ny+1), file_in);

  printf("AAAAA\n");
  
  foreach_vertex() {
    if( fabs(x)<=1.5 && fabs(y)<=1.5 ) {
      // Finding the index of the cell containing this point in the mesh
      int i_mesh = ROUND_TO_INT( (x - xMin)/dx );
      int j_mesh = ROUND_TO_INT( (y - yMin)/dy );

      phi[] = mesh_f[ i_mesh*(ny + 1) + j_mesh ];
    }
    else
      phi[] = -1000.0;
  }
  ...
  // Smoothing phi
  // for(int number_smoothings=10; number_smoothings>0; number_smoothings--) {
  //   foreach_vertex() {
  //     phi[] = 0.125*(phi[0, 0] + 
  //                 phi[1, 0] + phi[0, 1] + phi[-1, 0] + phi[0, -1] +
