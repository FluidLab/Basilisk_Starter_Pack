...
// To think: do we need any boundary conditions for the EVP polymeric stress tensor?
// Maybe check this with Mazi later...
...
event print_solution_mesh(t+=0.01) {
  if( t>=10 )
    return 0;
...
event print_solution_mesh_2(t+=1) {
  if( t<10 )
    return 0;
...
event init (t = 0) {  
  ...
  // adapt_wavelet ({f, u.x, u.y}, (double[]){1e-2, 1e-2, 1e-2}, maxlevel = mesh_level, minlevel = mesh_level-4);
  ...
  // refine( (f[]>=0.02) && (level<mesh_level-1) );
  // refine( (f[]>=0.02) && (level<mesh_level_inside) );
...
/// Setting the Saramito properties (viscosity and relaxation time)
event properties (i++) {
